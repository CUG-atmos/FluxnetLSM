...
GapfillMet_with_ERA <- function(datain, ERA_file, qc_name, varnames, site_log, ...){
  ...
  # `VPD` and `Qair` are the same, with the Fluxnet_variable of `VPD_F_MDS`. 
  # Hence, the length of `vpd_units` is 2, before apply `unique`.
  tair_units <- datain$units$original_units %>% .[names(.) %in% varnames$tair]
  vpd_units  <- datain$units$original_units %>% .[names(.) %in% varnames$vpd] %>% unique()

  #If not found, set to unknown
  if (is.na(tair_units) || length(tair_units) == 0) { tair_units = "UNKNOWN" }
  #If not found, assume hectopascals
  if (length(vpd_units) == 0 || is.na(vpd_units)) { vpd_units = "hPa" }
